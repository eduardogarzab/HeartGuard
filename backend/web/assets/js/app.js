/* HeartGuard Superadmin Panel (vanilla JS SPA)
   ...
   - Auto-refresh de access token al recibir 401
...
const CONFIG = {
  BASE_URL: localStorage.getItem("hg_base") || "", // vacío => same origin
};
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  envBase.addEventListener("click", () => {
    ...
    if (v === null) return;
    CONFIG.BASE_URL = v.trim().replace(/\/+$/, "");
...
/* ---------- AUTH STATE ---------- */
const auth = {
  get access() { return localStorage.getItem("hg_access") || ""; },
  set access(v) { v ? localStorage.setItem("hg_access", v) : localStorage.removeItem("hg_access"); },
  get refresh() { return localStorage.getItem("hg_refresh") || ""; },
  set refresh(v) { v ? localStorage.setItem("hg_refresh", v) : localStorage.removeItem("hg_refresh"); },
  get user() { try { return JSON.parse(localStorage.getItem("hg_user") || "null"); } catch { return null; } },
  set user(u) { u ? localStorage.setItem("hg_user", JSON.stringify(u)) : localStorage.removeItem("hg_user"); },
  isLogged() { return !!this.access && !!this.refresh; },
  clear() { this.access = ""; this.refresh = ""; this.user = null; },
};

function renderAuthArea() {
  const area = $("#authArea");
  if (!auth.isLogged()) {
    area.innerHTML = `<button class="btn success" id="goLogin"><span class="material-symbols-rounded">login</span> Iniciar sesión</button>`;
    $("#goLogin").onclick = () => { location.hash = "/login"; };
    return;
  }
  const u = auth.user || {};
  area.innerHTML = `
    <span class="user"><span class="material-symbols-rounded">shield_person</span>${u.email || "superadmin"}</span>
    <button class="btn danger" id="btnLogout"><span class="material-symbols-rounded">logout</span> Salir</button>
  `;
  $("#btnLogout").onclick = async () => {
    try {
      // Logout (revoca refresh). Si falla, igual limpiamos.
      await api("/v1/auth/logout", { method: "POST", data: { refresh_token: auth.refresh } }, { skipRetry: true });
    } catch {}
    auth.clear();
    renderAuthArea();
    location.hash = "/login";
    toast("Sesión", "Has cerrado sesión", "ok");
  };
}
renderAuthArea();
...
function modal({ title = "Confirmar", content = "", onOk = null, okText = "Confirmar", cancelText = "Cancelar" }) {
  ...
  $("#mOk").onclick = async () => { try { await onOk?.(); } finally { close(); } };
}

/* ---------- FETCH WRAPPER (con refresh automático) ---------- */
...
/**
 * api(path, options, meta)
 *  - options: { method, data, query }
 *  - meta: { skipRetry } -> si true, no intenta refresh en 401
 */
async function api(path, { method = "GET", data, query } = {}, meta = {}) {
  const url = buildURL(path, query);
  const headers = { "Content-Type": "application/json" };
  if (auth.isLogged()) headers["Authorization"] = `Bearer ${auth.access}`;

  let res = await fetch(url.toString(), { method, headers, body: data ? JSON.stringify(data) : undefined });
  if (res.status === 401 && !meta.skipRetry && auth.refresh) {
    // Intentar refresh y reintentar una vez
    try {
      const r = await fetch(buildURL("/v1/auth/refresh").toString(), {
        method: "POST",
      ...
      if (r.ok) {
        ...
        if (j.refresh_token) auth.refresh = j.refresh_token; // rotación
        // retry
        headers["Authorization"] = `Bearer ${auth.access}`;
        res = await fetch(url.toString(), { method, headers, body: data ? JSON.stringify(data) : undefined });
      } else {
        // refresh inválido -> forzar logout
        auth.clear(); renderAuthArea(); location.hash = "/login";
  ...
  if (!res.ok) {
    ...
    try { const j = JSON.parse(errText); errText = j.message || errText; } catch {}
...
const Router = {
  ...
  async render() {
    ...
    const route = this.routes[location.hash.replace(/^#/, "")] || this.routes["/"];
    const el = $("#app");
    el.innerHTML = "";
    try { await route(el); }
    catch (e) {
      console.error(e); toast("Error", e.message || String(e), "error");
...
Router.register("/login", async (el) => {
  el.innerHTML = `
    <div class="login-wrap">
      <div class="login-card">
        ...
        <p class="help">Introduce tus credenciales de superadmin.</p>
        <form id="loginForm" class="form-grid" autocomplete="on">
          <div class="form-field full">
            <label>Email</label>
            <input type="email" id="email" placeholder="admin@heartguard.com" required />
          </div>
          <div class="form-field full">
          ...
          <div class="login-actions">
            <button type="submit" class="btn success"><span class="material-symbols-rounded">login</span> Entrar</button>
          </div>
        </form>
      </div>
    </div>
  ...
  $("#loginForm").onsubmit = async (e) => {
    ...
    try {
      const data = await api("/v1/auth/login", { method: "POST", data: { email, password } }, { skipRetry: true });
      auth.access = data.access_token;
      auth.refresh = data.refresh_token;
      auth.user = data.user || null;
      renderAuthArea();
      location.hash = "/"; // a Home
      toast("Sesión", "Autenticación exitosa", "ok");
    } catch (err) {
      toast("Login", err.message, "error");
    }
  };
});
...
Router.register("/", async (el) => {
  el.innerHTML = `
    <div class="tile-grid">
      ...
      ${tile("Usuarios", "supervised_user_circle", "Búsqueda y estados", "#/users")}
...
function bindTabs(onChange) {
  $$(".tab").forEach((btn) => {
    btn.addEventListener("click", () => {
      const t = btn.dataset.t, idx = +btn.dataset.idx;
...
Router.register("/organizations", async (el) => {
  ...
  function create() {
    ...
    $("#orgCreate").onsubmit = async (e) => {
      ...
      try { await api("/v1/superadmin/organizations", { method: "POST", data: payload }); toast("Organización", "Creada", "ok"); await list(); }
  ...
  function update() {
    ...
    $("#orgUpdate").onsubmit = async (e) => {
      ...
      try { await api(`/v1/superadmin/organizations/${id}`, { method: "PATCH", data: payload }); toast("Organización", "Actualizada", "ok"); await list(); }
      catch (err) { toast("Error", err.message, "error"); }
    };
  }
  function remove() {
    ...
    $("#orgDelete").onsubmit = async (e) => {
      ...
      modal({
        ...
        okText: "Eliminar",
        onOk: async () => {
          try { await api(`/v1/superadmin/organizations/${id}`, { method: "DELETE" }); toast("Organización", "Eliminada", "ok"); await list(); }
          catch (err) { toast("Error", err.message, "error"); }
        },
      });
    };
...
Router.register("/users", async (el) => {
  ...
  el.innerHTML = panelScaffold("supervised_user_circle", "Usuarios",
    `<button class="btn ghost" onclick="location.hash='/'"><span class="material-symbols-rounded">arrow_back</span>Inicio</button>`, `${t.html}`);
  const panes = $("#" + t.id);
  ...
  function search() {
    ...
    $("#uSearch").onsubmit = async (e) => {
      e.preventDefault();
      const q = $("#q").value.trim(); const limit = +$("#limit").value || 20;
      try {
        const data = await api("/v1/superadmin/users", { query: { q, limit } });
        $("#uResults").innerHTML = `
          <table class="table"><thead><tr><th>ID</th><th>Nombre</th><th>Email</th><th>Status</th></tr></thead>
          <tbody>${(data || []).map(u => `
            <tr><td>${u.id}</td><td>${u.name || ""}</td><td>${u.email || ""}</td>
            <td><span class="badge ${u.status === "active" ? "success" : u.status === "blocked" ? "danger" : "warn"}">${u.status}</span></td></tr>`).join("")}
          </tbody></table>`;
      } catch (err) { toast("Error", err.message, "error"); }
    };
  }
  function status() {
    ...
    $("#uStatus").onsubmit = async (e) => {
      e.preventDefault();
      const id = $("#uid").value.trim(); const status = $("#ustatus").value;
      try { await api(`/v1/superadmin/users/${id}/status`, { method: "PATCH", data: { status } }); toast("Usuario", "Estado actualizado", "ok"); }
      catch (err) { toast("Error", err.message, "error"); }
    };
  }

  search(); bindTabs((tid, idx) => [search, status][idx]?.());
});

/* ---------- API KEYS ---------- */
Router.register("/apikeys", async (el) => {
  const t = tabs(["Listado", "Crear", "Permisos", "Revocar"]);
  el.innerHTML = panelScaffold("vpn_key", "API Keys",
    `<button class="btn ghost" onclick="location.hash='/'"><span class="material-symbols-rounded">arrow_back</span>Inicio</button>`, `${t.html}`);
  const panes = $("#" + t.id);
  ...
  async function list() {
    let data;
    if (filterMode === "active") data = await api("/v1/superadmin/api-keys", { query: { limit: 100, active_only: "true" } });
    else data = await api("/v1/superadmin/api-keys", { query: { limit: 100 } });

    data = data || [];
    if (filterMode === "revoked") data = data.filter(k => k.revoked || !!k.revoked_at || !!k.revokedAt);

    panes.innerHTML = `
      ${renderFilterBar()}
      <table class="table">
        <thead><tr><th>ID</th><th>Label</th><th>Owner</th><th>Estado</th><th>Expira</th><th>Creada</th></tr></thead>
        <tbody>${
          data.map(k => {
    ...
    panes.querySelectorAll('input[name="kfilter"]').forEach((r) => {
      r.addEventListener("change", async (e) => { filterMode = e.target.value; await list(); });
    });
  }
  ...
  function create() {
    ...
    $("#kCreate").onsubmit = async (e) => {
      ...
      try { await api("/v1/superadmin/api-keys", { method: "POST", data: payload }); toast("API Key", "Creada", "ok"); filterMode = "all"; await list(); }
      catch (err) { toast("Error", err.message, "error"); }
    };
  }
  ...
  function perms() {
    ...
    $("#kPerms").onsubmit = async (e) => {
      ...
      const permissions = $("#kperms").value.split(",").map((s) => s.trim()).filter(Boolean);
      try { await api(`/v1/superadmin/api-keys/${id}/permissions`, { method: "POST", data: { permissions } }); toast("API Key", "Permisos asignados", "ok"); }
      catch (err) { toast("Error", err.message, "error"); }
    };
  }
  ...
  function revoke() {
    ...
    $("#kDelete").onsubmit = async (e) => {
      ...
      modal({
        ...
        okText: "Revocar",
        onOk: async () => {
          try { await api(`/v1/superadmin/api-keys/${id}`, { method: "DELETE" }); toast("API Key", "Revocada", "ok"); await list(); }
          catch (err) { toast("Error", err.message, "error"); }
        },
      });
    };
  ...
  await list(); bindTabs((tid, idx) => [list, create, perms, revoke][idx]?.());
});

/* ---------- AUDIT LOGS ---------- */
Router.register("/audit", async (el) => {
  const t = tabs(["Listado", "Filtrar"]);
  el.innerHTML = panelScaffold("fact_check", "Logs de Auditoría",
    `<button class="btn ghost" onclick="location.hash='/'"><span class="material-symbols-rounded">arrow_back</span>Inicio</button>`, `${t.html}`);
  const panes = $("#" + t.id);
  ...
  async function list(params = {}) {
    ...
    panes.innerHTML = `
      <table class="table">
        ...
        <tbody>${(data || []).map(a => `
          <tr>
            <td>${new Date(a.ts || a.when || a.timestamp || Date.now()).toLocaleString()}</td>
            <td><span class="badge">${a.action}</span></td>
            <td>${a.user_id || "-"}</td>
            <td>${a.entity || "-"}</td>
            <td>${a.entity_id || "-"}</td>
            <td><code style="font-size:12px">${escapeHtml(JSON.stringify(a.details || {}, null, 0)).slice(0, 160)}</code></td>
          </tr>`).join("")}
        </tbody>
      </table>`;
  }
  function filter() {
    ...
    $("#aFilter").onsubmit = async (e) => {
      ...
      try { const data = await api("/v1/superadmin/audit-logs", { query: params }); $("#aRes").textContent = JSON.stringify(data, null, 2); }
      catch (err) { toast("Error", err.message, "error"); }
    };
  }
